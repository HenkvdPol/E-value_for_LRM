...
Sample_data <- function(stopping_value = 20, maximum_loops = 100){
  ...
  z_a <- rnorm(begin,1,2) # rep(1,begin) #    
  z_b <- rnorm(begin,1,2) # rep(2,begin) #  
  true_theta_x <- rnorm(begin,1,1/4) # rep(1/2, begin) # 
  true_theta_1 <- rnorm(begin,1,1/4) # rep(1/2, begin) # 
  ...
  for(i in 1:maximum_loops){
    ...
    # (ii) collect new data.
    z_a_new <- rnorm(1,1,2) # 1 # 
    z_b_new <- rnorm(1,1,2) # 2 # 
    true_theta_x_new = rnorm(1,1,1/4) # 1/2 #  
    true_theta_1_new = rnorm(1,1,1/4) # 1/2 # 
    ...
    theta_null <- estimate_coefficients(Y[(2*begin + 1):length(Y)], # i data points
                                        z_a[(begin + 1):length(z_a)], # without begin.
                                        z_b[(begin + 1):length(z_b)], 
                                        null = 0)
